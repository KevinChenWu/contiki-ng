...
features_list = ["node.id", "dio.version", "dio.rank", "frame.len", "6lowpan.src", "6lowpan.dst", "dio.dtsn", "dao.sequence", "ipv6.hlim", "wpan.seq_no", "ipv6.plen", "icmpv6.code", "wpan.ack_request"]
...
while True:
    if write_csv:
        with open(csv_file, mode="a", encoding="utf-8") as csv_file_handler:
            ...
            features_dict = {features_list[i]: data_list[i].strip() if int(data_header_bin[i-1]) or features_list[i] == "node.id" else "" for i in range(len(features_list))}
            csv_file_handler.write(",".join(features_dict.values()) + "\n")
            print(f"Node ID: {features_dict['node.id']}, Node IP: \"{address[0]}\", data_header: \"{data_header}\", data_header_bin: \"{data_header_bin}\", data_list: \"{data_list}\", features_dict: \"{features_dict}\"")
    else:
        data_bytes, address = udp_server.recvfrom(buffer_size)
